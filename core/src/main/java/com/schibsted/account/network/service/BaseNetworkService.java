...
import com.schibsted.account.util.CustomDeserializer;
...
public class BaseNetworkService {
    ...
    protected <T> T createService(@NonNull final Class<T> service) {
        ...
        final Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd")
                ...
                .registerTypeAdapter(new TypeToken<ResponseContainer<Subscription>>() {}.getType(), new CustomDeserializer<Subscription>())
                .registerTypeAdapter(new TypeToken<ResponseContainer<Product>>() {}.getType(), new CustomDeserializer<Product>())
